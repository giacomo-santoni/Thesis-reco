...
if __name__ == '__main__':
  ...
  for i in range(len(selectedEvents)):
    ...
    #***********************************2D PLOT***************************
    all_collinear_points = FindClosestToLinePoints(points, rho_thetas_max)
    ...
    for i, collinear_points in enumerate(all_collinear_points):
      print("ok")
      slope, intercept = np.polyfit(collinear_points[:,2], collinear_points[:,1], 1)
    ...
    # slope1, intercept1 = np.polyfit(all_collinear_points[0][:,2], all_collinear_points[0][:,1], 1)
    # print("m-q: ", slope1, intercept1)
    # theta_fit1 = - np.arctan(slope1)
    # rho_fit1 = intercept1*np.sin(theta_fit1)
    # print("(rho-theta) fit: ", rho_fit1, np.rad2deg(theta_fit1))
    # print("(rho-theta) hough: ", rho_thetas_max[0][0], np.rad2deg(rho_thetas_max[0][1]))
    # plt.plot(all_collinear_points[0][:,2], slope1*all_collinear_points[0][:,2] + intercept1, color='red')
    
    # slope2, intercept2 = np.polyfit(all_collinear_points[1][:,2], all_collinear_points[1][:,1], 1)
    # print("m-q: ", slope2, intercept2)
    # theta_fit2 = - np.arctan(slope2)
    # rho_fit2 = intercept2*np.sin(theta_fit2)
    # print("(rho-theta) fit: ", rho_fit2, np.rad2deg(theta_fit2))
    # print("(rho-theta) hough: ", rho_thetas_max[1][0], np.rad2deg(rho_thetas_max[1][1]))
    # print("par fit: ", slope2, intercept2)
    # plt.plot(all_collinear_points[1][:,2], slope2*all_collinear_points[1][:,2] + intercept2, color='orange')
